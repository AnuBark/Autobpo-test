...
// ========================================================================
// FINAL PATCHED: Grey.co + PayFast, HMAC, idempotency, safe JSON parsing,
// Twilio modern init, currency from milestone.
// ========================================================================
...
namespace DigitalCoFounder.BPO
{
    ...
    public class DeveloperMarketplaceService
    {
        ...
        {
            var devs = await _devRepo.Query().Where(d => d.IsAvailable && d.Verified).ToListAsync();
            return devs.OrderByDescending(d => d.Score).FirstOrDefault();
        }
    ...
    // ========================================================================
    // PaymentService – Grey.co + PayFast (fully patched)
    // ========================================================================
    ...
    public class PaymentService : IPaymentService
    {
        ...
        public PaymentService(IRepository<Payment> payRepo, IRepository<Developer> devRepo, IUnitOfWork uow, IEventBus bus, ILogger<PaymentService> logger, Config config, ITenantContext tenant, IConnectionMultiplexer redis)
        {
        ...
        public async Task<string> CreatePayFastRedirectUrlAsync(CreatePaymentIntentDto dto)
        {
            ...
            var merchantKey = _config.PayFastMerchantKey;
            ...
            var pfOutput = $"merchant_id={merchantId}&merchant_key={merchantKey}&return_url={returnUrl}&cancel_url={cancelUrl}&notify_url={notifyUrl}&amount={dto.Amount}&item_name=Project_{dto.ProjectId}&m_payment_id={paymentId}";
        ...
        public async Task HandlePayFastItnAsync(HttpRequest request)
        {
            ...
            if (status == "COMPLETE")
            {
                if (decimal.TryParse(form["amount"].ToString(), out var amount))
                {
    ...
    // ========================================================================
    // DecisionEngineService – with safe JSON parsing
    // ========================================================================
...
Return JSON: {{ ""score"": 85 }}";
        ...
        {
            if (!_hasOpenAI) return ("Proposal A: Experienced team available.", "Proposal B: Let's discuss your project!");
            try
            {
                var prompt = $"Write two proposals for job: {job.Title}\nBudget: {job.Budget}";
                var chat = _kernel.GetRequiredService<IChatCompletionService>();
                var result = await chat.GetChatMessageContentAsync(prompt);
                var parts = result.Content.Split(new[] { "Proposal B:" }, StringSplitOptions.None);
                return (parts[0].Replace("Proposal A:", "").Trim(), parts.Length > 1 ? parts[1].Trim() : "Second variant");
            }
            catch
            {
                return ("Proposal A: We can deliver high quality.", "Proposal B: Let's work together!");
            }
        }

        public async Task<decimal> EstimateScopeChangeCostAsync(Project project, string changeDescription)
        {
            if (!_hasOpenAI) return project.TotalPrice * 0.1m;
            try
            {
                var prompt = $"Estimate additional cost for change: {changeDescription}. Current budget: {project.TotalPrice}. Return only a number.";
                var chat = _kernel.GetRequiredService<IChatCompletionService>();
                var result = await chat.GetChatMessageContentAsync(prompt);
                if (decimal.TryParse(result.Content, out var cost)) return Math.Max(cost, 0);
                return 0;
            }
            catch
            {
    ...
    // ========================================================================
    // VoiceOnboardingService (modern Twilio)
    // ========================================================================
    ...
    public class SocialMediaService
    {
        ...
        public SocialMediaService(ILogger<SocialMediaService> logger, Config config)
        {
            _logger = logger; _config = config; _http = new HttpClient();
        }
    ...
    public interface IGitHubService
    {
        ...
        Task<string> GenerateReleaseZipAsync(int projectId, int milestoneId);
    ...
    public class GitHubService : IGitHubService
    {
        ...
        public async Task<string> GenerateReleaseZipAsync(int projectId, int milestoneId)
        {
            ...
            var fileName = $"project-{projectId}-milestone-{milestoneId}.zip";
    ...
    // ========================================================================
    // PayoutAccountService (renamed from StripeConnectService)
    // ========================================================================
    ...
    // ========================================================================
    // EscrowPaymentService (currency from milestone, uses IPaymentService)
    // ========================================================================
    public class EscrowPaymentService
    {
        ...
        public async Task<string> CreateMilestonePaymentIntentAsync(int milestoneId)
        {
            ...
            // Use ZAR as default – you can change to match project currency
            var currency = "ZAR";
    ...
    // ========================================================================
    // SubscriptionService (placeholder – can be extended with PayFast subscriptions)
    // ========================================================================
    public class SubscriptionService
    {
        ...
        public async Task<string> CreateCheckoutSessionAsync(string userId, int planId, string successUrl, string cancelUrl)
        {
            ...
            // Placeholder – replace with PayFast subscription logic
